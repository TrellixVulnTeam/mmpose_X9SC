...
def extract_pose_from_video():
    ...
    for video_name in video_names:
        label = int(video_name.split('/')[-1].split('A')[1][:3])
        ...
        while True:
            ...
            total_frame += 1
            if len(pose_results) == 1 and total_frame%3 == 0:
                img_num += 1
                ...
                video_poses.append(pose_results[0]["keypoints"])
        train_data.append(dict(label=label,video_name=video_name,keypoints=video_poses))
        print("video:{}  process finished".format(video_name))
